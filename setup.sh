...
envsubst < ./config/thorn-config.yaml > /tmp/thorn-config.yaml && $KUBECTL create configmap thorn-config --from-file /tmp/thorn-config.yaml -n ${NAMESPACE} -o yaml --dry-run=client | $KUBECTL apply -f -
